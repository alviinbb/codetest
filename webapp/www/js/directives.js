    ...
    return {
        link: function(scope, element) {
            var listener = function(transition) {
                ...
                if (transition.$to().data && transition.$to().data.pageTitle) title = 'App Portal | ' + transition.$to().data.pageTitle;
                $timeout(function() {
                    element.text(title);
                });
            };
...
function minimalizaSidebar($timeout) {
    return {
        restrict: 'A',
        template:   `<a class="navbar-minimalize minimalize-styl-2" ng-click=minimalize() style="margin: 10px 5px 5px 5px;">
